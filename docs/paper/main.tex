...
系统采用Flutter构建iOS与Android客户端，使用ArkTS与ArkUI构建HarmonyOS 6原生客户端，后端采用Rust、Axum、SeaORM与SQLite WAL。语言模型层默认接入DeepSeek，通过独立Provider配置与向量、语音识别和OCR能力解耦。端侧指标核心以C++稳定ABI封装，原始视频不离开终端，音频临时上传后删除，服务端只长期保存转写、报告与结构化指标。

项目将国产开放模型生态与HarmonyOS原生适配作为两项相互支撑的技术选择：云侧以可替换的DeepSeek接口处理材料约束下的语言理解，端侧面向我国自主移动操作系统生态建立原生交互与数据最小化边界，降低对单一模型供应商和单一移动平台的依赖。工程实现覆盖账号、项目、五类材料入口、持久化任务队列、知识库、训练会话、语音识别接口、可解释报告、AI评委、历史趋势、Flutter工作流、HarmonyOS原生工程、容器化部署与论文构建链。系统对未获得的端侧指标使用空值表达，并对供应商不可用、输出不合规和网络失败设置显式错误路径，不以伪造分析结果覆盖失败。
...
The implementation uses Flutter for iOS and Android, native ArkTS and ArkUI for HarmonyOS 6, and Rust with Axum, SeaORM, and SQLite WAL for the backend. DeepSeek is the default language-model provider and is configured independently from embedding, speech recognition, and OCR services. A shared C++ ABI defines observable edge metrics. The HarmonyOS client is treated as a native portability and privacy target rather than a web wrapper. Together, the replaceable domestic model interface and native HarmonyOS adaptation reduce dependence on a single model vendor or mobile platform. Unavailable dimensions remain nullable, and provider or validation failures are surfaced explicitly instead of being replaced with fabricated scores.
