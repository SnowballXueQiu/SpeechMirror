...
\path{AI_BASE_URL} & 兼容OpenAI协议的国产AI服务基址 \\
\path{AI_API_KEY} & 服务端供应商凭据 \\
\path{LLM_MODEL} & 问题生成、报告和回答评价模型名 \\
\path{EMBEDDING_MODEL} & 材料片段与查询向量化模型名 \\
\path{ASR_MODEL} & 训练与评委回答语音识别模型名 \\
\path{OCR_MODEL} & 扫描页面与图片文字识别模型名 \\
\bottomrule
\end{longtable}
